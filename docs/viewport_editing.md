...
- `Tools -> Import Prefab...` opens one import workspace instead of a
  sequence of prompts. Its searchable source browser lists `.ed` and `.dat`
  prefabs, and selecting a source shows format, geometry counts, bounds, model
  roles, source objects, ownership state, links, resource dependencies, and
  parse/behavioral diagnostics beside all import settings.
- `Static geometry` and `Full behavior` are explicit modes. The workspace
  independently reports `Static ready`, `Behavioral ready`, `Action required`,
  or `Blocked`; it never falls back from full behavior to static geometry.
  Object-only resource objects, passive mixed assemblies, owned moving BSP,
  linked graphs, hazards, destructibles, and shooters are currently supported.
  Linked imports cover
  `Trigger`, `Switch`, paired doors, compound movers, elevators, attachments,
  teleporters, damage/death targets, and compound traps. Phase 6 also reviews
  scripts by identity and content: `PropAnim.scr` is a name-independent
  pass-through, while the three Pipe Organ songs receive per-import rewritten
  copies. Unreviewed scripts remain blocked, and all blockers fail closed
  before placement.
- Linked prefabs expose target-level binding fields in the workspace. External
  object names must already exist in the level. Portal names must already be
  compiled into its VisBSP; enter `<omit>` to deliberately clear a prefab's
  `PortalName` when dynamic visibility culling is not required. Authored ED
  portal helper brushes cannot be merged into an existing VisBSP and are not
  imported as visible BSP geometry.
- Literal object lookups inside reviewed scripts use the same binding fields.
  Pipe Organ note names are rewritten to the imported namespace; its external
  `Bell1` through `Bell5` targets must already exist in the level. Generated
  song scripts are written to a complete staged `SCRIPTS.REZ`, never to the
  installed archive.
- The workspace keeps BSP name, placement anchor, collision strategy, box
  dimensions, and static-only behavior acknowledgement together. Invalid
  sources/options and target-level name/schema conflicts are reported inline.
  `Start placement` closes the workspace and the next BSP click places the
  configured preview. Surface placement defaults to the prefab's bottom-center;
  the original origin, bounds center, and a resolvable controller pivot remain
  selectable.
- Source `.ed` imports combine visible brush records and compile them into one
  independent MM9 BSP submodel while retaining texture paths and OPQ texture
  projection. Resource-only prefabs with no brushes are not static BSP assets
  and are rejected with a specific explanation.
- Static prefab import prefers ordinary/controller geometry, falls back to
  `PhysicsBSP` for system-only converted prefabs, and never imports `VisBSP`
  implicitly. `VisBSP` leaf/PVS data is source-world-specific and is blocked
  unless a caller explicitly enables the diagnostic override.
- Authored collision imports a compiled prefab's `PhysicsBSP`, or the solid
  brush subset of a source `.ed`, under a hidden `InvisibleBrush`. Box
  approximation is an explicit fallback generated by the editor and does not
  depend on an existing collision brush in the target level.
- A separate same-named `WorldObject` is created for every imported visible BSP
  submodel. Its property schema, and the `InvisibleBrush` schema when needed,
  come from `object.lto` catalog metadata stored with the pending operation.
- Static import does not copy scripts or behavioral object graphs. Prefabs that
  contain controller objects require an explicit geometry-only confirmation.
...
- Behavioral prefabs become one atomic `ImportBehavioralPrefabOp`; the same
  operation owns their objects, links, resource decisions, scripts, and BSP.
...
- Moving, rotating, or deleting any pending behavioral-prefab member updates or
  removes its whole atomic assembly. This preserves paired-door spacing and
  internal links.
- Property panel edits use `EditOp` for existing objects and override updates
  for pending added objects.

`CloneDoorOp` remains in these internal edit paths only for compatibility with
older `.mm9mod` projects. The editor no longer creates new clone operations.
...
- Set `MM9_EDITOR_PROFILE_LOAD=1` to print editor-level and viewport-stage
  timings once whenever a level is opened or selected. This separates asset,
  materialization, object-model, BSP upload, sky, camera-fit, and object-panel
  costs without enabling continuous frame profiling.
